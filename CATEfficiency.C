...
//      by  b_branchname->GetEntry(ientry); //read only this branch
	...
	int count = 0;
	float array1[] = {3,23,43,63,83,103};
	float array2[] = {3,19,35,51,67,83};
	float array3[] = {3,7,11,15,19,23};
	float array4[] = {2,4,6,8,10};
	float array5[] = {2,4,6,8};
	float array6[] = {2,4,6,8};
	for (float a : array1) {
		for (float b  : array2) {
			for (float c  : array3) {
				for (float d  : array4) {
					for (float e  : array5) {
						for (float f  : array6) {
							if (count >= in1 && count < in2) {
								...
								std::cout<<cuts[1]<<" "<<cuts[2]<<" "<<cuts[3]<<" "<<cuts[7]<<" "<<cuts[8]<<" "<<cuts[9]<<" "<<100*primaryTpMRet/primaryTpMatchesSize<<" "<<100*tpMRet/tpMatchesSize<<" "<<100 - ((goodCut*100.)/goodTracks)<<" "<<100 - ((badCut*100.)/badTracks)<<std::endl;
							} else if (count >= in2)
								break;
							++count;
						}
...
void	CATEfficiency::point(vector<float> cuts, bool highPt) {
	...
	std::cout<<cuts[1]<<" "<<cuts[2]<<" "<<cuts[3]<<" "<<cuts[7]<<" "<<cuts[8]<<" "<<cuts[9]<<" "<<100*primaryTpMRet/primaryTpMatchesSize<<" "<<100*tpMRet/tpMatchesSize<<" "<<100 - ((goodCut*100.)/goodTracks)<<" "<<goodCut<<" "<<goodTracks<<" "<<100 - ((badCut*100.)/badTracks)<<" "<<badCut<<" "<<badTracks<<std::endl;
}
