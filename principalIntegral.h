#ifndef principalIntegral_h
#define principalIntegral_h
...
public :
   ...
   std::vector<vector<unsigned int> > *AMTTRoads_superstripIds;
   std::vector<vector<vector<unsigned int> > > *AMTTRoads_stubRefs;
   std::vector<float>   *AMTTTracks_pt;
   std::vector<float>   *AMTTTracks_preEstimatedPt;
   ...
   std::vector<vector<unsigned int> > *AMTTTracks_stubRefs;
   std::vector<vector<float> > *AMTTTracks_principals;
   ...
   TBranch	  *b_AMTTTracks_preEstimatedPt;   //!
   ...
   principalIntegral(TTree *tree=0);
   virtual ~principalIntegral();
   virtual Int_t    Cut();
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     loop(bool highPt);
...
#ifdef principalIntegral_cxx
principalIntegral::principalIntegral(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
//TString file = "/fdata/scratch/rathjd/JacobTransfer/NewMergedBanks03082016_MuonPU140_nz8_ACB-PDDS3_NoDR_NoClones_fixedPtEstimate.root";
TString file = "/fdata/scratch/rathjd/JacobTransfer/OldSingleElectron_ACB-PDDS3_NoDR.root";
//TString file = "/home/jacob.a.griffin/RicardoCurve_merged_SCB-PDDS3_NoCut_NoDR.root";
...
principalIntegral::~principalIntegral()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t principalIntegral::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t principalIntegral::LoadTree(Long64_t entry)
{
...
void principalIntegral::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("AMTTTracks_preEstimatedPt", &AMTTTracks_preEstimatedPt, &b_AMTTTracks_preEstimatedPt);
...
Bool_t principalIntegral::Notify()
{
...
void principalIntegral::Show(Long64_t entry) {
...
Int_t principalIntegral::Cut() {
// This function may be called from loop.
// returns  1 if entry is accepted.
// returns -1 otherwise.
   return 1;
}
#endif // #ifdef principalIntegral_cxx
