#ifndef CATEfficiency_h
#define CATEfficiency_h
...
public :
   ...
   CATEfficiency(TTree *tree=0);
   virtual ~CATEfficiency();
   virtual Int_t    Cut();
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     loop(int in1, int in2, bool highPt);
   virtual void     point(vector<float> cuts, bool highPt);
...
#ifdef CATEfficiency_cxx
CATEfficiency::CATEfficiency(TTree *tree) : fChain(0) 
{
...
CATEfficiency::~CATEfficiency()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t CATEfficiency::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t CATEfficiency::LoadTree(Long64_t entry)
{
...
void CATEfficiency::Init(TTree *tree)
{
...
void CATEfficiency::Show(Long64_t entry) {
...
Int_t CATEfficiency::Cut() {
// This function may be called from loop.
// returns  1 if entry is accepted.
// returns -1 otherwise.
   return 1;
}
#endif // #ifdef CATEfficiency_cxx
